  ...
  theme: {
    ...
    extend: {
      ...
      fontFamily: {
        ...
        satoshi: ["var(--font_satoshi)"],
      },

      keyframes: {
        "accordion-down": {
          from: { height: "0" },
          to: { height: "var(--dss-accordion-content-height)" },
        },
        "accordion-up": {
          from: { height: "var(--dss-accordion-content-height)" },
          to: { height: "0" },
        },
        moveHorizontal: {
          "0%": {
      ...
      matchUtilities(
        {
          ...
          "bg-noise": (value: any) => ({
            backgroundImage: `url("${svgToDataUri(
              `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" width="16" height="16" fill="none"><circle fill="black" id="pattern-circle" cx="10" cy="10" r="1.6257413380501518"></circle></svg>`
            )}")`,
          }),
        },
